...
class CustomerControllerIntegrationTest {
    ...
    fun setup() {
        ...
        customer = userRepository.save(User(id = "cust_1", username = "cust", password = passwordEncoder.encode("pass"), role = Role.CUSTOMER, fullName = "Test Customer", email = "cust@test.com", phone = "+96812345678", branchId = branch.id))
    }

    @Test
    fun `should list customers as admin`() {
        mockMvc.perform(get("/api/customers").with(httpBasic("adm", "pass")))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.content[0].fullName").value("Test Customer"))
    }

    @Test
    fun `should get customer detail`() {
        mockMvc.perform(get("/api/customers/${customer.id}").with(httpBasic("adm", "pass")))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.fullName").value("Test Customer"))
    }

    @Test
    fun `should reject customer access to customer list`() {
        mockMvc.perform(get("/api/customers").with(httpBasic("cust", "pass")))
